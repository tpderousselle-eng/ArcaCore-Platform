...
FORBIDDEN_LABELS=frozenset({"tenant_id","request_id","object_id","email","url","filename","exception","message"})
...
class MetricDefinition:
 ...
 def __post_init__(self):
  ...
  if tuple(sorted(set(self.labels)))!=self.labels or any(not _NAME.fullmatch(v) or v in FORBIDDEN_LABELS for v in self.labels):raise ValueError("Metric labels are unsafe.")
  if self.kind==MetricKind.HISTOGRAM and (not self.buckets or tuple(sorted(set(self.buckets)))!=self.buckets):raise ValueError("Histogram buckets are invalid.")
...
class Tracer:
 def __init__(self,exporter,max_depth=16,known_secrets=()):
  ...
  self.exporter=exporter;self.max_depth=max_depth;self.secrets=tuple(known_secrets)
 def start(self,name,context,*,parent=None,attributes=None):
  ...
  for forbidden in ("request_body","response_body","sql","sql_parameters"):
   if forbidden in safe:raise ValueError("Sensitive span attribute is forbidden.")
