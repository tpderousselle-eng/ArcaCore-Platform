...
class LoggingTest(unittest.TestCase):
 ...
 def test_recursive_secret_and_header_redaction(self):
  self.logger.emit("ERROR","job.failed","jobs",CorrelationContext("c"),"bad s3cr3t",{"authorization":"Bearer x","nested":{"value":"s3cr3t"},"exception":"s3cr3t"});self.assertNotIn("s3cr3t",self.out[0]);self.assertNotIn("Bearer",self.out[0])
